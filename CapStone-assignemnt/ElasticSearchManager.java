...
public class ElasticSearchManager {
    ...
    private static final String INDEX = "12312343342"; // TODO: MUST CHANGE THIS to random number string
     ...
     * Returns all remote users from server
     */
    public static class GetBidListTask extends AsyncTask<Void,Void,ArrayList<User>> {
        ...
        protected ArrayList<User> doInBackground(Void... params) {
            ...
            try {
                SearchResult execute = client.execute(search);
                if (execute.isSucceeded()) {
                    List<Bid> remote_users = execute.getSourceAsObjectList(Bid.class);
                    bids.addAll(remote_users);
    ...
    public static class AddBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        @Override
        protected Boolean doInBackground(B... params) {
    ...
    public static class RemoveBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(Bid... params) {
           ...
           Bid bid = params[0];
            try {
                DocumentResult execute = client.execute(new Delete.Builder(bid.getId()).index(INDEX).type(USER_TYPE).build());
                if(execute.isSucceeded()) {
